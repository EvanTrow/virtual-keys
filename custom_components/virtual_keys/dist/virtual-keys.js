...
class VirtualKeysPanel extends LitElement {
          ...
          user.tokens
            ...
            .forEach((token) => {
              this.tokens.push({
                ...
                dashboard_path: token.dashboard_path,
                kiosk: token.kiosk,
              });
            });
        });
  ...
  dashboardChanged(e) {
    this.dashboardPath = e.target.value;
  }
  ...
  addClick() {
    ...
    this.hass
      .callWS({
        ...
        dashboard_path: this.dashboardPath,
        kiosk: this.kiosk,
      })
      .then(() => {
        this.fetchUsers();
  ...
  getLoginUrl(token) {
    let url =
      ...
      token.jwt_token;
    if (token.dashboard_path) {
      url += "&path=" + encodeURIComponent(token.dashboard_path);
    }
    if (token.kiosk) {
      url += "&kiosk=1";
    }
        ...
        <div class="mdc-top-app-bar--fixed-adjust flex content">
          <div class="filters">
            ...
            <ha-textfield
              ...
              .value="${this.dashboardPath}"
              .required=${false}
              @input="${this.dashboardChanged}"
              placeholder="/lovelace/default or /"
            ></ha-textfield>

            <ha-button @click=${this.toggleKiosk} size="small">
              ${this.kiosk ? "Kiosk: On" : "Kiosk: Off"}
            </ha-button>
